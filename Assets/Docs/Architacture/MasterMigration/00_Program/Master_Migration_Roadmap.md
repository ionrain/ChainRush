...
Phase 4 next slice (after this checkpoint): continue `C04B` (`multi-scope / multi-arbiter host restructure`) before broad multi-faction rollout. `C04C` (domain-orchestrator form convergence) is closed as a domain-shape milestone; `C04D` is the follow-up structural extraction that moves generic orchestration composition abstractions out of `Morboo.Integration.StrategyCombat` into existing upper packages (`RuntimeHost` / `Core` / `Framework` as appropriate). Further `StrategyCombat` route-policy/profile evolution must follow `C04D` ownership boundaries and must not reintroduce RuntimeHost ownership of route bodies.
...
11. `Assets/Docs/Architacture/MasterMigration/04_Phase4_Orchestration/C04D_Generic_Orchestration_Composition_Extraction_2026-02-23.md` -> planned structural extraction of generic orchestration composition abstractions out of `Morboo.Integration.StrategyCombat` into existing upper packages (`RuntimeHost` / `Core` / `Framework` by ownership; no compatibility path; no `Base` suffix for abstract upper-layer types).
...
10. `C05` activate domain event pipeline.
