...
1. **`C06B` — TargetProvider/DomainComponent convergence + reservations on top of `TargetRef`**
   ...
   - Explicitly converge `CombatTargetProvider` and `IdleTargetProvider` to a shared domain-neutral provider/output contract on top of `DomainTargetProvider` + `DomainTargetSet` (not just a shared base type), so `CombatDomainComponent` and `IdleDomainComponent` can use one structural domain-output pipeline pattern.
   ...
   - Architecture analysis/decision for post-convergence seam ownership: after `DomainComponent` + `DomainTargetSet` + (`CombatTargetProvider`/`IdleTargetProvider`) convergence, evaluate whether `DomainTargetSet` should be reusable across domains (default target-candidate carrier), whether provider seams stay separate or partially merge into domain components, and whether `DomainComponent` should remain a separate seam or collapse into `DomainOrchestrator`/`DomainOrchestratorComponent`. Record and implement one chosen direction in `C06B` (no long-lived half-state).
