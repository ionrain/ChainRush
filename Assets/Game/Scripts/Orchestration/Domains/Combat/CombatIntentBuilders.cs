...
public static class CombatIntentBuilders
{
    ...
    public static Intent Hold(FactionKey faction, int priority = 50, string tag = null)
    {
        return Create(CombatGoalId.Hold, faction, priority, tag: tag);
    }

    public static Intent AdvanceTo(FactionKey faction, Vector2 point, int priority = 50, float deadlineSeconds = -1f, string tag = null)
    {
        return Create(CombatGoalId.Advance, faction, priority, deadlineSeconds, targetPoint: point, tag: tag);
    }

    public static Intent Defend(FactionKey faction, Vector2 point, int priority = 60, float deadlineSeconds = -1f, string tag = null)
    {
        return Create(CombatGoalId.Defend, faction, priority, deadlineSeconds, targetPoint: point, tag: tag);
    }

    public static Intent Eliminate(FactionKey faction, UnityEngine.Object target, int priority = 70, float deadlineSeconds = -1f, string tag = null)
    {
        return Create(CombatGoalId.Eliminate, faction, priority, deadlineSeconds, target: target, tag: tag);
    }

    public static Intent Support(FactionKey faction, UnityEngine.Object target, int priority = 50, string tag = null)
    {
        return Create(CombatGoalId.Support, faction, priority, target: target, tag: tag);
    }

    public static Intent Retreat(FactionKey faction, Vector2 point, int priority = 80, string tag = null)
    {
