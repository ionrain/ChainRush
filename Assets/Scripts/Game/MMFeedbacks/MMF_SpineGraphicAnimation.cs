...
/// </summary>
[AddComponentMenu("")]
[FeedbackHelp("This feedback will simply play the specified Spine Grpahic Animation.")]
[FeedbackPath("Spine/Spine Graphic Animation")]
public class MMF_SpineGraphicAnimation : MMF_Feedback {
    ...
    protected override void AutomateTargetAcquisition() => SpineAnimator = FindAutomatedTarget<SkeletonGraphic>();
    ...
    public SkeletonGraphic SpineAnimator;
    ...
    protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1) {
        if (!Active || !FeedbackTypeAuthorized)
            return;
        if (Mode == Modes.Set)
            SpineAnimator.AnimationState.SetAnimation(Track, Name, Loop); 
        else 
            SpineAnimator.AnimationState.AddAnimation(Track, Name, Loop, Delay);

        if (!Loop && AddEmpty)
            SpineAnimator.AnimationState.AddEmptyAnimation(Track, StopMixDuration, EmptyDelay);
    }

    protected override void CustomStopFeedback(Vector3 position, float feedbacksIntensity = 1.0f) {
        SpineAnimator.AnimationState.SetEmptyAnimation(Track, StopMixDuration);
    }
