...
/// <summary>
/// Maps <see cref="RoleAsset"/> to <see cref="ActorCapabilityProfile"/>.
...
/// </para>
/// </summary>
[CreateAssetMenu(fileName = "RoleActorCapabilitiesMap", menuName = "Game/Orchestration/Role Actor Capabilities Map")]
public sealed class RoleActorCapabilitiesMapAsset : RoleActorCapabilitiesMapAssetBase
{
    [Serializable]
    public struct Entry
    {
    ...
    /// <summary>
    /// Looks up the <see cref="ActorCapabilityProfile"/> for a given <see cref="RoleAsset"/>.
    ...
    /// </summary>
    public override bool TryGet(RoleAsset role, out ActorCapabilityProfile profile)
    {
        profile = null;
    ...
    Dictionary<RoleId, ActorCapabilityProfile> _byRoleId;

    /// <summary>
    /// Lookup by <see cref="RoleId"/> for RuntimeHost code.
    /// IMPORTANT: RuntimeHost must use this overload, never the RoleAsset one.
    /// </summary>
    public override bool TryGet(RoleId roleId, out ActorCapabilityProfile profile)
    {
    ...
    {
        _byRoleId = new Dictionary<RoleId, ActorCapabilityProfile>(entries != null ? entries.Count : 0);
    ...
    void OnValidate()
    {
        ...
        for (int i = 0; i < entries.Count; i++)
        {
            if (entries[i].Role == null)
                Debug.LogWarning($"[RoleActorCapabilitiesMap] Entry {i} has null RoleAsset.", this);
            if (entries[i].Profile == null)
                Debug.LogWarning($"[RoleActorCapabilitiesMap] Entry {i} has null ActorCapabilityProfile.", this);

            // Duplicate check
            for (int j = i + 1; j < entries.Count; j++)
            {
                if (entries[i].Role != null && ReferenceEquals(entries[i].Role, entries[j].Role))
                    Debug.LogWarning($"[RoleActorCapabilitiesMap] Duplicate RoleAsset '{entries[i].Role.Id}' at entries {i} and {j}.", this);
            }
        }
    }
#endif
