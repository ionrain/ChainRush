...
/// <summary>
/// Data-driven mapping of <see cref="RoleAsset"/> to combat targeting policies.
/// Assigned to <see cref="OrchestrationArbiter"/> to define which targeting policy
/// each unit role uses during combat.
/// <para>
...
/// </para>
/// </summary>
[CreateAssetMenu(fileName = "CombatRolePolicyMap", menuName = "Game/Orchestration/Combat/Role Policy Map")]
public sealed class CombatRolePolicyMapAsset : ScriptableObject
{
    [Serializable]
    public struct Entry
    {
        public RoleAsset role;
        public CombatTargetingPolicyAsset policy;
    }
    ...
    /// </summary>
    public bool TryGet(RoleAsset role, out CombatTargetingPolicyAsset policy)
    {
        if (role == null)
        {
            policy = null;
    ...
    Dictionary<RoleId, CombatTargetingPolicyAsset> _byRoleId;
    ...
    /// </summary>
    public bool TryGet(RoleId roleId, out CombatTargetingPolicyAsset policy)
    {
    ...
    {
        _byRoleId = new Dictionary<RoleId, CombatTargetingPolicyAsset>(entries != null ? entries.Count : 0);
    ...
    void OnValidate()
    {
        ...
        for (int i = 0; i < entries.Count; i++)
        {
            ...
            {
                Debug.LogWarning(
                    "[CombatRolePolicyMapAsset] Null role at index " + i + ".", this);
                continue;
            }
            ...
            {
                Debug.LogWarning(
                    "[CombatRolePolicyMapAsset] Null policy for role '" + role.Id +
                    "' at index " + i + ".", this);
            }
            ...
            for (int j = i + 1; j < entries.Count; j++)
            {
                ...
                {
                    Debug.LogWarning(
                        "[CombatRolePolicyMapAsset] Duplicate role '" + role.Id +
                        "' at indices " + i + " and " + j +
                ...
                {
                    Debug.LogWarning(
                        "[CombatRolePolicyMapAsset] Different RoleAssets with same Id '" + role.Id +
                        "' at indices " + i + " and " + j +
