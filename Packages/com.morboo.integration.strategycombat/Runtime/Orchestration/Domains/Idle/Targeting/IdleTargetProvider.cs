...
/// </summary>
public sealed class IdleTargetProvider : DomainTargetProvider, IDomainOperatorPositionProvider
{
    ...
    public bool TryResolveOperatorPosition(
        EntityId operatorEntityId,
        OrchestrationWorldCache world,
        ExecutionContext ctx,
        out Float3 position)
    {
        position = ResolveSelfPositionInternal(operatorEntityId, world, ctx, useActorReadProjection);
        return true;
    }

    public Float3 ResolveSelfPosition(EntityId entityId, OrchestrationWorldCache world, ExecutionContext ctx)
    {
        return ResolveSelfPositionInternal(entityId, world, ctx, useActorReadProjection);
    }

    static Float3 ResolveSelfPositionInternal(EntityId entityId, OrchestrationWorldCache world, ExecutionContext ctx, bool useActorReadProjection)
    {
